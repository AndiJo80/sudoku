						...
						Button {
							...
							do {
								guard let saveData = data.first else {
									throw SaveDataError(.noSaveData, "No save data exists.")
								}
								Logger.debug("Save data: \(saveData)")
								try boardData.generatePuzzle(saveData: saveData)
								boardData.prepareBoard()
							} catch let error as SaveDataError {
								Logger.error("Cannot continue game because save data has errors. \(error.details)")
							} catch {
